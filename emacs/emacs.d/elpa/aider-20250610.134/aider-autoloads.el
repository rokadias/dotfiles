...
Make direct code change given user's prompt." t)
...
(autoload 'aider-function-or-region-refactor "aider-code-change" "\
Refactor code under cursor or in selected region.
If a region is selected, refactor that specific region.
Otherwise, refactor the function under cursor." t)
...
(autoload 'aider-magit-log-analyze "aider-git" "\
...
save it to 'PROJECT_ROOT/git.log', open this file, and then analyze its content." t)
...
`aider-software-planning--sequential-thinking-prompt`." t)
